"""
integrate_pspire_pdb_region.py

Merges PSPire pdb_region scores into:
  1. output/topology_scores_raw/PSPire_all_approaches.csv   (appends pdb_region rows)
  2. output/topology_scores_master.csv                       (rebuilds from build script)

Run after run_pspire_pdb_region.py completes:
...
SRC     = RAW_DIR / "PSPire_pdb_region_raw.csv"
DEST    = RAW_DIR / "PSPire_all_approaches.csv"
MASTER  = ROOT / "build_topology_scores_master.py"

if not SRC.exists():
    sys.exit(f"Not found: {SRC} — run run_pspire_pdb_region.py first")
...
# ── Append to PSPire_all_approaches.csv ────────────────────────────────────
...
print(f"Updated PSPire_all_approaches.csv: {len(combined)} rows")
