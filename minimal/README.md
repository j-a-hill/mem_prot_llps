...
A stripped-down version of this project: get the databases, build one table, plot
and test. Plain pandas and matplotlib, no package to import, no framework. Every
script runs on its own and prints what it did.
...
Five LLPS databases, one UniProt annotation file:

| source | what it gives | file used |
|---|---|---|
| CD-CODE | condensate membership, role words (driver/member) | `output/cdcode/cdcode_proteins_all.json` |
| PhaSepDB | membership, PS-self / PS-other class | `exp_db/phasepdb_summary_database_2026-06-15.csv` |
| DrLLPS | membership, Scaffold / Regulator / Client type | `catGRANULE2.0_current/.../DrLLPS/LLPS.csv` (tab-separated) |
| PhaSePro | membership, partner dependence | `LLPS_DB_data/phasepro.txt` |
| LLPSDB | membership only (no role field) | `exp_db/protein_LLPSDB.xls` |
| UniProt | TRANSMEM / INTRAMEM features → the membrane filter | `data/uniprot_tm_cache.csv` |

`01_download.py` tries each URL, then **parses what came back** and keeps it only if it
contains at least a plausible number of records. Anything else falls back to the local
snapshot, with the reason recorded. `raw/PROVENANCE.csv` logs origin, record count and
fallback reason per source, so a run is always traceable.

The parse check is the point of the script, not a nicety. A download can return HTTP
200 at a plausible size and still be useless, and all three failure modes are live here:

- **PhaSepDB** serves an HTML page from its download URL, not a table.
- **CD-CODE**'s API is paginated 25 records at a time, so one GET returns 25 of 11,144
  — a perfectly valid JSON file that would silently shrink the set by 99%.
- **UniProt** serves TSV while the stored snapshot is CSV.

A size check catches none of these. On a typical run only UniProt downloads cleanly
(20,431 rows, slightly ahead of the 20,366-row snapshot); the other five fall back.
The set size is unchanged either way, which is the useful thing to know.
...
Running the 18 LLPS predictors is a separate, slow job. This pipeline treats their
output as an **input**, joined on UniProt accession from
`v2.0/output/figures/union475_v3/output/`. Three files come from there:
...
**Always read these from the per-run directory, never from the top-level
`v2.0/output/`.** The top-level copies are legacy 427-protein files and mixing them in
caused a real erratum in this project. Fast discriminator: the canonical clean-mask
table has 475 rows, the stale one 427. `06_background.py` asserts this on startup.
...
Two things in the current paper are deliberately absent:
...
- **Feature correlations (paper Fig 3).** The 14-feature Spearman panel. Not rebuilt
  ...
  would roughly double this pipeline. `positron/llps/stats.py` has it, and the
  verified table is `positron/data/tables/feature_correlations_both_sets.csv`.
- **The evidence ladder (paper Fig 5).** Local LR+ by the Pejaver method — adaptive
  ...
  needs its own careful treatment rather than a compressed version. The verified table
  is `positron/data/tables/evidence_calibration_pejaver_method.csv`.

Both are read-only inputs if you want to plot them from minimal without re-deriving.
