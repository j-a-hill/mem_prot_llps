# Helpers — LLPS analysis
#
# Mirrors the classification logic in llps/functional.py and llps/location.py.
# Keep LOCATION_CANONICAL and FUNCTION_CATEGORIES in sync with:
#   data/functional_classification_terms.yaml  (Python source of truth)
#   r_shiny/R/data.R                           (shiny app copy)
...
LOCATION_CANONICAL <- list(
  ...
  list(kws = c("nucleus", "nucleoplasm", "nucleolus", "nuclear pore",
               "nuclear envelope"),                                               name = "Nucleus"),
  ...
  list(kws = c("cell junction", "tight junction", "adherens junction",
               "gap junction"),                                                   name = "Cell junction"),
...
FUNCTION_CATEGORIES <- list(
  ...
  "Phosphatase" = c(
    "\\bphosphatase\\b", "\\bprotein\\s+phosphatase\\b",
    "\\bdephosphorylates\\b", "\\bpten\\b"
  ),
  "Protease" = c(
  ...
  "Hydrolase" = c(
    "\\bhydrolase\\b", "\\besterase\\b", "\\blipase\\b",
    "\\batpase\\b", "\\bhydrolytic\\b"
  ),
  ...
  "Chromatin remodeling" = c(
    ...
    "\\bhistone.*demethylase\\b", "\\bepigenetic\\b",
    "\\bdna\\s+methyltransferase\\b", "\\bnucleosome.*remodeling\\b"
  )
)
...
parse_functions <- function(func_str, name_str = NA_character_) {
  ...
  combined <- tolower(gsub("\\{[^}]*\\}", "", paste(parts, collapse = " ")))
  found <- character(0)
